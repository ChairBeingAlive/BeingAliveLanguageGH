...
namespace GSP {
  public static class NativeBridge {
    ...
    [DllImport(WinLibName, EntryPoint = "BALpossionDiskElimSample",
               CallingConvention = CallingConvention.Cdecl)]
    private static extern bool BALpossionDiskElimSampleWin(byte[] inBuffer, int inSize,
                                                           double generalArea, int dim, int n,
                                                           out IntPtr outBuffer, out int outSize);
    [DllImport(MacLibName, EntryPoint = "BALpossionDiskElimSample",
               CallingConvention = CallingConvention.Cdecl)]
    private static extern bool BALpossionDiskElimSampleMac(byte[] inBuffer, int inSize,
                                                           double generalArea, int dim, int n,
                                                           out IntPtr outBuffer, out int outSize);
    public static bool BALpossionDiskElimSample(byte[] inBuffer, int inSize, double generalArea,
                                                int dim, int n, out IntPtr outBuffer,
                                                out int outSize) {
      try {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
          return BALpossionDiskElimSampleWin(inBuffer, inSize, generalArea, dim, n, out outBuffer,
                                             out outSize);
        else
          return BALpossionDiskElimSampleMac(inBuffer, inSize, generalArea, dim, n, out outBuffer,
                                             out outSize);
      } catch (Exception ex) {
