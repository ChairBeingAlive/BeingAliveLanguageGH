...
namespace BeingAliveLanguage {
  ...
  public static class ClipperUtils {
    public static Polyline OffsetPolygon(in Plane pln, in Polyline polyIn, in double ratio) {
      ...
      var res =
          Clipper.InflatePaths(polyPath, dis, JoinType.Miter, EndType.Polygon, Math.Abs(dis) * 10);
  ...
  public static class SamplingUtils {
    static public List<Point3d> uniformSampling(ref SoilBase sBase, int num) {
      ...
      // calculate approximate radius, based on the max density 0.9069:
      ...
      while (true) {
        pt2d = FastPoisson
                   .GenerateSamples((float)(sBase.bnd.Width), (float)(sBase.bnd.Height),
                                    (float)approxR)
                   .ToList();
  ...
  public static class FastPoisson {
    private static int _k =
        30;  // recommended value from the paper TODO provide a means for configuring this value
    ...
    /// <summary>
    ///     Generates a Poisson distribution of <see cref="Vector2"/> within some rectangular shape
    ///     defined by <paramref name="height"/> * <paramref name="width"/>.
    /// </summary>
    ...
    /// <returns>Enumeration of <see cref="Vector2"/> elements where no element is within <paramref
    /// name="radius"/> distance to any other element.</returns>
    public static IEnumerable<System.Numerics.Vector2> GenerateSamples(float width, float height,
                                                                       float radius) {
      List<System.Numerics.Vector2> samples = new List<System.Numerics.Vector2>();
      // Random random = new Random(); // TODO evaluate whether this Random can generate uniformly
      // random numbers

      // cell size to guarantee that each cell within the accelerator grid can have at most one
      // sample
      ...
      while (activeSamples.Count != 0) {
        ...
        bool isValid =
            false;  // need to keep track whether or not the sample we have meets our criteria
        ...
        for (int i = 0; i < _k; i++) {
          ...
          float distance =
              Utils.balRnd.Next((int)(radius * 100), (int)(2 * radius * 100)) / (float)100.0;
          ...
          isValid = IsGeneratedSampleValid(generatedSample, width, height, radius, cellSize,
                                           samples, accelerator);

          if (isValid) {
            activeSamples.Add(generatedSample);  // we may be able to add more samples around this
                                                 // valid generated sample later
            samples.Add(generatedSample);

            // mark the generated sample as "taken" on our accelerator
            accelerator[(int)(generatedSample.X / cellSize), (int)(generatedSample.Y / cellSize)] =
                samples.Count;
    ...
    private static bool IsGeneratedSampleValid(System.Numerics.Vector2 generatedSample, float width,
                                               float height, float radius, float cellSize,
                                               List<System.Numerics.Vector2> samples,
                                               int[,] accelerator) {
      // is our generated sample within our boundaries?
      if (generatedSample.X < 0 || generatedSample.X >= height || generatedSample.Y < 0 ||
          generatedSample.Y >= width) {
      ...
      // TODO - for some reason my math for initially have +/- 2 for the area bounds causes some
      // points to just slip
      //        through with a distance just below the radis - bumping this up to +/- 3 solves it at
      ...
      for (int x = startX; x <= endX; x++) {
        for (int y = startY; y <= endY; y++) {
          ...
          if (index >= 0)  // in each point for the accelerator where we have a sample we put the
                           // current size of the number of samples
          {
            // compute Euclidean distance squared (more performant as there is no square root)
            float distance =
                System.Numerics.Vector2.DistanceSquared(generatedSample, samples[index]);
      ...
      return true;  // this is a valid generated sample as there are no other samples too close to
                    // it
    }
  }
}
