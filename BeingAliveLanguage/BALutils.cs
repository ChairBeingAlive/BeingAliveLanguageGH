...
namespace BeingAliveLanguage {
  public class BALsoilInfo : GH_Component {
    public BALsoilInfo()
        : base("Soil_Information", "balSoilInfoText", "Export the soil information in text format.",
               "BAL", "09::utils") {}

    public override GH_Exposure Exposure => GH_Exposure.primary;
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilInfo);
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      pManager.AddTextParameter("Soil Info Text", "soilText",
                                "Soil Info that can be visualized with the TAG component.",
                                GH_ParamAccess.item);
    }
  ...
  public class BALmorphFan : GH_Component {
    public BALmorphFan()
        : base("Bound Morph (Fan)", "balMorphFan",
               "Morph the soil diagram from a rectangle shape into a fan shape.", "BAL",
               "09::utils") {}

    public override GH_Exposure Exposure => GH_Exposure.primary;
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilMorphFan);
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      pManager.AddGenericParameter("Soil Base", "soilBase",
                                   ...
                                   GH_ParamAccess.item);
      pManager.AddCurveParameter("Soil Geo", "soilG",
                                 "Any line- or polyline-based geometries from the soil system.",
                                 GH_ParamAccess.list);

      pManager.AddCurveParameter("Target Fan", "tFan",
                                 "The target closed fan shape curve to morph to.",
                                 GH_ParamAccess.item);
    }

    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      pManager.AddCurveParameter("Transformed Geo", "fSoilG",
                                 "The morphed soil geometries inside the given fan shape.",
                                 GH_ParamAccess.list);
      pManager.AddPlaneParameter(
          "Transformed Plane", "fP",
          "The based plane for the polar coordinate system used during the mapping process",
          GH_ParamAccess.item);
    }
    ...
    protected override void SolveInstance(IGH_DataAccess DA) {
      ...
      if (!DA.GetData("Soil Base", ref sBase)) {
        AddRuntimeMessage(GH_RuntimeMessageLevel.Warning,
                          "no soil base detected, mapping results might be incorrect.");
      }
      ...
      foreach (var crv in soilG) {
        if (crv.TryGetPolyline(out Polyline polyln)) {
          ...
          AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                            "Soil geometries contain non-polyline-based object(s).");
          return;
        }
      }
