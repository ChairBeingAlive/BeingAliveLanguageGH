...
public class BALETP : GH_Component {
  public BALETP()
      : base("Climate_HydraulicBalance",
             "balClimate_HydraBal",
             "Calculate the climatical hydralic balance data and relevant information " +
                 "(evapotranspiration, etc.) for a given location.",
             "BAL",
             "05::climate") {}
  ...
  protected override void RegisterInputParams(GH_InputParamManager pManager) {
    pManager.AddNumberParameter(
        "Precipitation", "P", "Precipitation of given location in 12 months.", GH_ParamAccess.list);
    pManager.AddNumberParameter(
        "Temperature", "T", "Temperature of given location in 12 months.", GH_ParamAccess.list);
    pManager.AddNumberParameter(
        "Latitude", "Lat", "Latitude of the given location.", GH_ParamAccess.item);
    // pManager.AddNumberParameter("MaxReserve", "maxRes", "Maximum reserved water of previous
    // years.", GH_ParamAccess.item);
    pManager.AddGenericParameter("SoilInfo",
                                 "soilInfo",
                                 "Info about the current soil based on given content ratio.",
                                 GH_ParamAccess.item);
    pManager.AddNumberParameter(
        "SoilDepth", "soilDep", "The depth [m] of the target soil.", GH_ParamAccess.item, 0.5);
    pManager[4].Optional = true;
  }

  protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
    pManager.AddNumberParameter("Potential Evapotranspiration (Corrected)",
                                "PET.corr",
                                "Corrected evapotranspiration (mm/yr).",
                                GH_ParamAccess.list);
    pManager.AddNumberParameter("Actual Evapotranspiration",
                                "ETa",
                                "Real evapotranspiration (mm/yr).",
                                GH_ParamAccess.list);
    pManager.AddNumberParameter("Surplus",
                                "SUR",
    ...
    pManager.AddNumberParameter("MaxReserve",
                                "maxRES",
                                "The maximum ammount of water can be reserved in the soil (mm). " +
                                    "When this value is reached, the soil is fully saturated.",
                                GH_ParamAccess.item);
  }
...
public class BALGaussen : GH_Component {
  ...
  protected override void RegisterInputParams(GH_InputParamManager pManager) {
    pManager.AddPlaneParameter(
        "Plane", "pln", "The plane to draw the diagram.", GH_ParamAccess.item, Plane.WorldXY);
    pManager[0].Optional = true;

    pManager.AddNumberParameter("Precipitation",
                                "Prec",
                                "Precipitation of given location in 12 months. Right click to " +
                                    "switch between SI [mm] and Imperial system [in].",
                                GH_ParamAccess.list);
    pManager[1].Optional = true;
    pManager.AddIntervalParameter("Precipitation Range",
                                  "Prec-range",
                                  "Range of the precipitation. Omit to use auto-scaling feature.",
                                  GH_ParamAccess.item);
    pManager[2].Optional = true;

    pManager.AddNumberParameter("Temperature",
                                "Temp",
                                "Temperature of given location in 12 months. Right click to " +
                                    "switch between SI (°C) and Imperial system (°F).",
                                GH_ParamAccess.list);
    pManager[3].Optional = true;
    pManager.AddIntervalParameter("Temperature Range",
                                  "Temp-range",
  ...
  protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
    pManager.AddCurveParameter(
        "Frame", "F", "The Frame of the Gaussen diagram.", GH_ParamAccess.list);

    pManager.AddCurveParameter(
        "TempCrv", "TC", "The temperature curve of the Gaussen diagram.", GH_ParamAccess.list);
    pManager.AddCurveParameter(
        "PrecCrv", "PC", "The precipitation curve of the Gaussen diagram.", GH_ParamAccess.list);

    pManager.AddPlaneParameter("LabelLocation",
                               "TxtLoc",
                               "The label location of the Gaussen diagram.",
                               GH_ParamAccess.tree);
    pManager.AddGenericParameter(
        "Label", "Txt", "The label text of the Gaussen diagram.", GH_ParamAccess.tree);
    // pManager.AddPlaneParameter("testPln", "tP", "xxxx", GH_ParamAccess.item);
  }

  protected override void
  AppendAdditionalComponentMenuItems(System.Windows.Forms.ToolStripDropDown menu) {
  ...
  protected override void SolveInstance(IGH_DataAccess DA) {
    ...
    if (tempRng.T1 > tempRng.T0) {
      ...
      maxTemp =
          (tempLst.Max() > highTemps[0] ? tempLst.Max() > highTemps[1] ? highTemps[2] : highTemps[1]
                                        : highTemps[0]);
    ...
    List<double> tempHeight =
        tempLst
            .Select(x => Utils.remap(
                        x, minTemp, tempLabelInterval.Max(), diagramVertL, tempHeightUpperLimit))
            .ToList();
