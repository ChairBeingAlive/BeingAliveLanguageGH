...
namespace BeingAliveLanguage
{
  ...
  public class Tree3D
  {
    ...
    {
      // Pass 1: Compute raw scale factors for all branch nodes.
      var scaleFactors = new Dictionary<int, double>();
      foreach (var branch in mAllNode)
      {
        if (branch == mBaseNode)
          continue;
        scaleFactors[branch.mID] = CalculateScaleFactor(branch);
      }

      // Pass 2: Smooth scale factors among sibling branches so that adjacent
      // branches don't have abrupt transitions (e.g., one fully scaled, neighbor
      // untouched). Influence propagates with angular falloff.
      PropagateScaleToSiblings(scaleFactors);

      // Pass 3: Apply smoothed scale factors. mAllNode is ordered parent-before-child,
      // so when a parent is scaled its entire sub-tree inherits the factor via
      // ScaleBranchAndSubBranches. Deeper nodes already marked as scaled are skipped.
      var scaledBranches = new HashSet<int>();
      foreach (var branch in mAllNode)
      {
        if (branch == mBaseNode)
          continue;
        if (scaledBranches.Contains(branch.mID))
          continue;
        if (!scaleFactors.TryGetValue(branch.mID, out double factor))
          continue;

        if (factor < 1.0)
        {
          ScaleBranchAndSubBranches(branch, factor, scaledBranches);
        }
      }
    }

    /// <summary>
    /// For each group of sibling branches (children of the same parent), propagate
    /// scaling influence from affected branches to their angular neighbors.
    /// Uses cos²(angle/2) falloff: full influence when pointing in the same direction,
    /// zero when pointing in opposite directions.
    /// </summary>
    private void PropagateScaleToSiblings(Dictionary<int, double> scaleFactors)
    {
      // Propagation strength: at max angular alignment, adopt this fraction
      // of the sibling's scaling effect. 0.7 gives a natural gradient.
      const double propagationStrength = 0.7;

      foreach (var rel in mBranchRelation)
      {
        var childIds = rel.Value.ToList();
        if (childIds.Count < 2)
          continue;

        // Compute local-plane 2D direction for each sibling branch.
        // Branches with negligible horizontal extent (e.g., the top virtual node)
        // are excluded — they have no directional identity to compare.
        var childDirs = new Dictionary<int, Vector3d>();
        foreach (var id in childIds)
        {
          var node = mAllNode.FirstOrDefault(n => n.mID == id);
          if (node == null || node.mBranch.Count == 0)
            continue;

          mPln.RemapToPlaneSpace(node.mBranch[0].PointAtEnd, out Point3d local);
          var dir = new Vector3d(local.X, local.Y, 0);
          if (dir.Length > 0.001)
          {
            dir.Unitize();
            childDirs[id] = dir;
        ...
        // For each sibling, check if any angular neighbor has a lower (more aggressive)
        // scale factor and pull this branch's factor toward it.
        var smoothed = new Dictionary<int, double>();
        foreach (var id in childDirs.Keys)
        {
          double ownFactor = scaleFactors.ContainsKey(id) ? scaleFactors[id] : 1.0;
          double result = ownFactor;

          foreach (var sibId in childDirs.Keys)
          {
            if (sibId == id)
              continue;
            double sibFactor = scaleFactors.ContainsKey(sibId) ? scaleFactors[sibId] : 1.0;
            if (sibFactor >= result)
              continue;  // only pull toward smaller (more scaled) values

            double angle = Vector3d.VectorAngle(childDirs[id], childDirs[sibId]);

            // cos²(angle/2) = (1 + cos(angle)) / 2
            // Smooth falloff: 1.0 at same direction, ~0.75 at 60°, ~0.25 at 120°, 0 at 180°
            double weight = 0.5 * (1.0 + Math.Cos(angle));
            if (weight < 0.001)
              continue;

            // Blend toward sibling's factor proportional to weight and propagation strength
            double propagated = ownFactor - (ownFactor - sibFactor) * weight * propagationStrength;
            result = Math.Min(result, propagated);
          }
        ...
        foreach (var s in smoothed)
        {
          scaleFactors[s.Key] = s.Value;
        }
      }
    }
    ...
    private double CalculateScaleFactor(BranchNode3D node)
    {
      ...
      // Calculate the maximum reach of this branch hierarchy in the tree's local plane.
      // Using RemapToPlaneSpace ensures correct results regardless of plane orientation.
      ...
      while (branchQueue.Count > 0)
      {
        ...
        {
          // Calculate 2D distance from tree center to branch endpoint in tree's local plane
          mPln.RemapToPlaneSpace(branchCurve.PointAtEnd, out Point3d localEnd);
          double dist2D = Math.Sqrt(localEnd.X * localEnd.X + localEnd.Y * localEnd.Y);
      ...
      // Calculate the direction of the main branch from tree center in the tree's local plane
      mPln.RemapToPlaneSpace(node.mBranch[0].PointAtEnd, out Point3d localBranchEnd);
      Vector3d branchDir2D = new Vector3d(localBranchEnd.X, localBranchEnd.Y, 0);
      ...
      {
        // Direction from tree center to neighbor in tree's local plane
        mPln.RemapToPlaneSpace(treePt, out Point3d localTreePt);
        Vector3d treeDir2D = new Vector3d(localTreePt.X, localTreePt.Y, 0);
