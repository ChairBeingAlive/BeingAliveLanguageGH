...
class SoilGeneral {
  ...
  public SoilGeneral(in SoilBase sBase,
                     in SoilProperty sInfo,
                     in List<Curve> stone,
                     in int seed,
                     in int stage = 5) {
    mBase = sBase;
  ...
  public void Build() {
    ...
    } else  // RndControl
    {
      if (mStage == 0) {
         ...
         * rest from random sampling. 100% will cause all clay/silt triangle accumulated to the edge
         * if sand ratio > 90%
         */
    ...
    // #region method 2
    //   sample general points, then find the corresponding triangles, final results not as clean as
    //   method 1
...
class SoilUrban {
  public SoilUrban(in SoilBase sBase,
                   in double rSand,
                   in double rClay,
                   in double rBiochar,
                   in List<double> rStone,
                   in List<double> szStone) {
    this.sBase = sBase;
    this.rSand = rSand;
    this.rClay = rClay;
...
#region Sand
    ...
    if (totalASand > 0) {
      ...
      // will overlap
      // var kdMap = new KdTree<double, Polyline>(2, new KdTree.Math.DoubleMath(),
      // AddDuplicateBehavior.Skip); foreach (var pl in postSandT)
      //{
      //    var cen = (pl[0] + pl[1] + pl[2]) / 3;
      //    var originalCen = cen;
      //    cen.Transform(toLocal);
...
#region clay, biochar
  ...
  PickAndCluster(in List<Polyline> polyIn, List<double> ratioLst, List<double> szLst) {
    ...
    // convert relative stone radii for generating distributed points  24 ~ 64 triangles makes small
    // ~ big stones
    ...
    // BeingAliveLanguageRC.Utils.SampleElim(sBase.bnd, stoneCntLst.Sum(), out genCen, out stoneCen,
    // -1, 0.93);
    ...
    // collect the initial triangle we use a new struct "StoneCluster" to store info related to the
    // final stones
...
#region Initialize Stone Collection
    ...
    for (int i = 0; i < ratioLst.Count; i++) {
      ...
      // BeingAliveLanguageRC.Utils.SampleElim(tmpStoneCen, sBase.bnd.Area, stoneCntLst[i], out
      // curLst);
    ...
    List<double> stoneTypeArea =
        Enumerable.Repeat(0.0, ratioLst.Count).ToList();  // store the total area of each stone type
    ...
    while (!areaReached) {
      // the recordArea is used to guarantee that when stoneTypeArea cannot expand to targetArea, we
      // also stop safely.
      ...
      foreach (var i in stoneIndices) {
        ...
        string bestT = SelectBestTriangle(stoneCol[i].strIdNeigh,
  ...
  /// </summary>
  private string SelectBestTriangle(HashSet<string> candidates,
