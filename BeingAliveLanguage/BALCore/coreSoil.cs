...
namespace BeingAliveLanguage {
  class SoilGeneral {
    ...
    public void Build() {
      ...
      // var kdMap = new KdTree<double, Point3d>(2, new KdTree.Math.DoubleMath(),
      // AddDuplicateBehavior.Skip); foreach (var pl in triL)
      //{
      ...
      } else  // RndControl
      {
        if (mStage == 0) {
          ...
          /*  part 1
           * convert stage to randomness param: 1-10 --> 5% - 95% from Poisson's Disk Sampling, the
           * rest from random sampling. 100% will cause all clay/silt triangle accumulated to the
           * edge if sand ratio > 90%
           */
          int numPoissonSand = Convert.ToInt32(numSand * Utils.remap(mStage, 1.0, 8.0, 1.0, 0.05));
          // BeingAliveLanguageRC.Utils.SampleElim(triCen, mBase.bnd.Area, numPoissonSand, out
          // outSandCen);
          ...
          var randomTriCen =
              remainingTriCen.OrderBy(x => rnd.Next()).Take(numSand - numPoissonSand);
      ...
      // #region method 2
      //   sample general points, then find the corresponding triangles, final results not as clean
      //   as method 1
      // List<Point3d> genPt = new List<Point3d>();
      // List<Point3d> sampledPt = new List<Point3d>();
      // BeingAliveLanguageRC.Utils.SampleElim(mBase.bnd, numSand, out genPt, out sampledPt, mSeed,
      // 1, mStage);
      ...
      } else  // with RndControl
      {
        ...
        // BeingAliveLanguageRC.Utils.SampleElim(preSiltCen, mBase.bnd.Area, numPoissonSilt, out
        // outSiltCen);
        ...
        var curRandomTriCen =
            curRemainTriCen.OrderBy(x => rnd.Next()).Take(numSilt - numPoissonSilt);
  ...
  class SoilUrban {
    public SoilUrban(in SoilBase sBase, in double rSand, in double rClay, in double rBiochar,
                     in List<double> rStone, in List<double> szStone) {
      this.sBase = sBase;
      this.rSand = rSand;
      this.rClay = rClay;
...
#region clay, biochar
    ...
    /// <summary>
    /// Main func: pick and cluster lv3 triangles into stones, according to the input sz and ratio
    /// list
    ...
    /// </summary>
    public List<Polyline> PickAndCluster(in List<Polyline> polyIn, List<double> ratioLst,
                                         List<double> szLst) {
      ...
      List<double> areaLst =
          ratioLst.Select(x => x * totalArea).ToList();  // the target area for each stone type
      HashSet<string> allTriCenStr = new HashSet<string>(cenMap.Keys);
      HashSet<string> pickedTriCenStr = new HashSet<string>();

      // build a kd-map for polygon centre. We need to transform into 2d, otherwise, collision box
      // will overlap
      var kdMap =
      ...
      // convert relative stone radii for generating distributed points  24 ~ 64 triangles makes
      // small ~ big stones
      var stoneSzTriLst = szLst.Select(x => (int)Utils.remap(x, 1, 5, 24, 64)).ToList();
      var stoneCntLst =
          areaLst.Zip(stoneSzTriLst, (a, n) => (int)Math.Round(a / (singleTriA * n))).ToList();
      ...
      // BeingAliveLanguageRC.Utils.SampleElim(sBase.bnd, stoneCntLst.Sum(), out genCen, out
      // stoneCen, -1, 0.93);
      // TODO: integrate seed number
      cppUtils.SampleElim(sBase.bnd, stoneCntLst.Sum(), out genCen, out stoneCen, mSeed, 0.98);

      // ! separate the stoneCen into several clusters according to the number of stone types, and
      // collect the initial triangle we use a new struct "StoneCluster" to store info related to
      // the final stones
      var stoneCol = new List<StoneCluster>(stoneCen.Count);

      // ! Explanation:
      /// to decide the number of stones for each ratio, we actually need to solve a linear
      /// programming question of:
      ///
      /// Sum(N_i) = N
      ...
      ///
      ///
      /// However, for the two stone type case, we can direct solve the only solution without the
...
#region Initialize Stone Collection
      ...
      // for (int i = ratioLst.Count - 1; i >= 0; i--) // reverse the order, from bigger elements to
      // smaller
      ...
      // ! start to aggregate stone triangles and boolean into bigger ones. The target area for each
      // cluster is stoneArea[i]
      bool areaReached = false;
      List<double> stoneTypeArea = Enumerable.Repeat(0.0, ratioLst.Count)
                                       .ToList();  // store the total area of each stone type
      ...
      stoneIndices = stoneIndices
                         .OrderBy(
                             _ => Guid.NewGuid())
                         .ToList();

      while (!areaReached) {
        // the recordArea is used to guarantee that when stoneTypeArea cannot expand to targetArea,
        // we also stop safely.
        ...
        foreach (var i in stoneIndices) {
          ...
          string bestT = SelectBestTriangle(stoneCol[i].strIdNeigh, stoneCol[i].strIdInside,
                                            pickedTriCenStr, stoneCol[i].cen, stoneCol[i].distMap);
    ...
    /// <summary>
    /// Calculate the number of shared edges between a candidate triangle and the existing stone
    /// cluster. More shared edges = triangle fills a concave gap = smoother shape.
    /// </summary>
    ...
    /// </summary>
    private string SelectBestTriangle(HashSet<string> candidates, HashSet<string> stoneTriangles,
                                      HashSet<string> pickedTriangles, Point3d stoneCen,
      ...
      foreach (var candidateId in candidates) {
        ...
        if (sharedEdges > bestSharedEdges) {
          ...
          if (compactness >
              bestCompactness + 0.01)  // Small threshold to avoid floating point issues
          {
            isBetter = true;
