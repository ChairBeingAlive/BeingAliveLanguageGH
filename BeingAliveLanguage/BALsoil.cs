...
namespace BeingAliveLanguage {
  ...
  public class BALsoilAnalysis : GH_Component {
    public BALsoilAnalysis()
        : base("Soil Analysis", "balSoilAna",
               "Analysis the soil composition and determine the soil information.", "BAL",
               "01::soil") {}

    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilAnalysis);
    public override Guid ComponentGuid => new Guid("F12AEDCA-4FE1-4734-8E71-249C9D90CBA1");
    public override GH_Exposure Exposure => GH_Exposure.primary;

    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      pManager.AddNumberParameter("Sand Ratio", "rSand", "The ratio of sand in the soil.",
                                  GH_ParamAccess.item, 1.0);
      pManager.AddNumberParameter("Silt Ratio", "rSilt", "The ratio of silt in the soil.",
                                  GH_ParamAccess.item, 0.0);
      pManager.AddNumberParameter("Clay Ratio", "rClay", "The ratio of clay in the soil.",
                                  GH_ParamAccess.item, 0.0);
    }
    ...
    protected override void SolveInstance(IGH_DataAccess DA) {
      ...
      if (Math.Abs(rSand + rClay + rSilt - 1) > 1e-4 || rSand < 0 || rClay < 0 || rSilt < 0) {
        AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                          "Ratio of all content need to sum up to 1. Only positive ratio allowed.");
        return;
      }
  ...
  public class BALsoilBase : GH_Component {
    public BALsoilBase()
        : base("Soil Base", "balSoilBase", "Generate a base map from the boundary rectangle.",
               "BAL", "01::soil") {}

    public override Guid ComponentGuid => new Guid("27d616d0-c23c-4ae6-8108-ecbcb3d9125a");
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilBase);

    public string resMode = "vertical";

    protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager) {
      pManager.AddRectangleParameter("Boundary", "Bound", "Boundary rectangle.",
                                     GH_ParamAccess.item);
      pManager.AddIntegerParameter(
          "Resolution", "res", "Vertical resolution of the generated grid.", GH_ParamAccess.item);
      pManager.AddBooleanParameter(
          "Scale", "scale",
          "Scale the generated grid to fill the boundary. (note: the triangles will not be regular triangle.)",
          GH_ParamAccess.item, false);
    }

    protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager) {
      pManager.AddGenericParameter("Soil Base", "soilBase",
                                   "The base object used for soil diagram generation.",
                                   GH_ParamAccess.item);
      pManager.AddGenericParameter("Soil Base Grid", "soilT",
                                   "The base grids used for soil diagram generation.",
                                   GH_ParamAccess.list);
    }
    ...
    public override void AppendAdditionalMenuItems(ToolStripDropDown menu) {
      ...
      Menu_AppendItem(menu, "Resolution Mode:", (sender, e) => {}, false).Font =
          GH_FontServer.StandardItalic;
      Menu_AppendItem(menu, "horizontal",
                      (sender, e) => Menu.SelectMode(this, sender, e, ref resMode, "horizontal"),
                      true, CheckMode("horizontal"));
      Menu_AppendItem(menu, " vertical",
                      (sender, e) => Menu.SelectMode(this, sender, e, ref resMode, "vertical"),
                      true, CheckMode("vertical"));
    }
  ...
  public class BALsoilDiagramGeneral : GH_Component {
    public BALsoilDiagramGeneral()
        : base(
              "General Soil Separates", "balsoilGeneral",
              "Draw a soil map based on the ratio of 3 soil separates, and avoid rock area rocks if rock curves are provided.",
              "BAL", "01::soil") {}

    public BALsoilDiagramGeneral(string name, string nickname, string description, string category,
                                 string subCategory)
        : base(name, nickname, description, category, subCategory) {}

    public override GH_Exposure Exposure => GH_Exposure.secondary;
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilDiv);
    public override Guid ComponentGuid => new Guid("9ffd9134-0d95-4e4d-859d-753df5b8dd4a");

    protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager) {
      pManager.AddGenericParameter("Soil Base", "soilBase", "soil base triangle map.",
                                   GH_ParamAccess.item);
      pManager.AddGenericParameter("Soil Info", "soilInfo",
                                   "Info about the current soil based on given content ratio.",
                                   GH_ParamAccess.item);

      pManager.AddCurveParameter("Rocks", "R", "Curves represendting the rocks in the soil.",
                                 GH_ParamAccess.list);
      ...
      pManager.AddIntegerParameter("seed", "s",
                                   "Int seed for randomize the generated soil pattern.",
    ...
    protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager) {
      ...
      pManager.AddCurveParameter("All Triangle", "soilT",
                                 "Collection of all triangles of the three types.",
                                 GH_ParamAccess.list);
    }
  ...
  public class BALsoilDiagramGeneral_RndControl : BALsoilDiagramGeneral {
    public BALsoilDiagramGeneral_RndControl()
        : base(
              "General Soil Separates (RndControl)", "balsoilGeneral_rndControl",
    ...
    public override GH_Exposure Exposure => GH_Exposure.secondary;
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilDiv_rnd);
    ...
    protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager) {
      ...
      pManager.AddIntegerParameter("seed", "s", "Int seed to randomize the generated soil pattern.",
                                   GH_ParamAccess.item, -1);
      pManager[3].Optional = true;  // if no seed is provided, use random seeds

      pManager.AddIntegerParameter(
          "stage", "t",
          "Int stage index [1 - 8] representing the randomness of the soil separates that are gradually changed by the organic matter.",
    ...
    protected override void SolveInstance(IGH_DataAccess DA) {
      ...
      //{
      //   AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "The MacOS version component does not
      //   allow the adjustment of randomness levels in the soil generation process.");
      ...
      // if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
      //{
      //   AddRuntimeMessage(GH_RuntimeMessageLevel.Error, "This component is only available for the
      //   Windows platform.");
  ...
  public class BALsoilDiagramUrban : GH_Component {
    ...
    protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager) {
      ...
      pManager.AddNumberParameter("Biochar Ratio", "rBiochar", "The ratio of biochar in the soil.",
                                  GH_ParamAccess.item, 0);
      pManager.AddNumberParameter("Stone Ratio", "rStone", "The ratio of stone in the soil.",
                                  GH_ParamAccess.list, 0);
      pManager.AddNumberParameter(
          "Relative Stone Size", "szStone",
          "The relative stone size [1, 5], representing stones dia. from 5mm to 50mm in reality.",
          GH_ParamAccess.list, 3);
      pManager.AddNumberParameter("Organic Matter Ratio", "rOM",
                                  "The ratio of organic matter in the soil.", GH_ParamAccess.item,
                                  0);
      // TODO: if we should separate organic matter out
      pManager[6].Optional = true;  // rock can be optionally provided
      pManager.AddIntegerParameter("Seed", "s", "Int seed to generate soil pattern.",
                                   GH_ParamAccess.item, -1);
      pManager.AddNumberParameter("Offset Ratio", "rOff",
                                  "Temp solution for offset ratio based on water storage ability.",
                                  GH_ParamAccess.item, 0.5);
    }

    protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager) {
      // pManager.AddGenericParameter("Soil Info", "soilInfo", "Info about the current soil based on
      // given content ratio.", GH_ParamAccess.item);
      ...
      pManager.AddCurveParameter("Biochar Tri", "biocharT", "Biochar triangles.",
                                 GH_ParamAccess.list);
      pManager.AddCurveParameter("Stone Poly", "stonePoly", "Stone polygons.", GH_ParamAccess.tree);
      pManager.AddLineParameter("Organic Matter", "OM", "Collection of organic matters.",
                                GH_ParamAccess.list);
      pManager.AddCurveParameter("Non-offset Tri", "allT", "Collection of all polygons.",
                                 GH_ParamAccess.list);
    }
    ...
    protected override void SolveInstance(IGH_DataAccess DA) {
      ...
      if (rSand > 0 && rStone.Sum() > 0) {
        AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                          "No urban stone contains sand and stone simultaneously.");
        return;
      }
      if (rSand + rClay + rBiochar + rOM + rStone.Sum() != 1) {
        AddRuntimeMessage(
            GH_RuntimeMessageLevel.Error,
            $"Ratio of all contents need to sum up to 1. Current value is {rSand + rClay + rBiochar + rOM + rStone.Sum()}");
        return;
      }
      if (szStone.Any(x => x < 1 || x > 5)) {
        AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                          "Relative stone size out of range [1 - 5].");
        return;
      }
      if (rStone.Count != szStone.Count) {
        AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                          "# of Stone ratios and sizes need to be matched.");
        return;
      }
      ...
      //{
      //     var path = new GH_Path(i);
      //     stoneColTree.AppendRange(urbanS.stoneCollection[i].Select(x => new
      //     GH_Curve(x.ToPolylineCurve())), path);
      // }
      // DA.SetDataTree(idx++, stoneColTree);
    }

    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilUrban);
  ...
  public class BALsoilWaterOffset : GH_Component {
    public BALsoilWaterOffset()
        : base("Soil Water Visualization", "balSoilWaterVis",
               "Generate soil diagram with water info.", "BAL", "01::soil") {}

    public override GH_Exposure Exposure => GH_Exposure.tertiary;
    public override Guid ComponentGuid => new Guid("320a54cc-ca9a-44a2-b313-2ba08035cb1c");
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilWaterVis);
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      ...
      pManager.AddNumberParameter(
          "Current Water ratio", "rCurWater",
          "The current water ratio [0, 1] in the soil for visualization purposes.",
          GH_ParamAccess.item, 0.5);
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      pManager.AddCurveParameter(
          "Soil Core", "soilCore",
          "Soil core triangles, representing soil separates without any water.",
          GH_ParamAccess.list);
      pManager.AddCurveParameter("Wilting Point", "soilWP", "Soil wilting point triangles.",
                                 GH_ParamAccess.list);
      pManager.AddCurveParameter("Field Capacity", "soilFC", "Soil field capacity triangles.",
                                 GH_ParamAccess.list);
      pManager.AddCurveParameter("Saturation", "soilST", "Soil saturation triangles.",
                                 GH_ParamAccess.list);

      pManager.AddCurveParameter("Current Water Content", "soilCW", "Current water stage.",
                                 GH_ParamAccess.list);
    }
  ...
  public class BALsoilOrganicMatterInner : GH_Component {
    public BALsoilOrganicMatterInner()
        : base("Soil Interior Organic Matter", "balSoilOG_in",
               "Generate soil inner organic matter based on given intensity.", "BAL", "01::soil") {}
    ...
    protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager) {
      ...
      pManager.AddCurveParameter(
          ...
          "Soil triangles, can be any or combined triangles of sand, silt, clay.",
          GH_ParamAccess.list);
      pManager.AddNumberParameter("Organic Matter Density", "dOrganics",
                                  "Density of organic matter [ 0 - 1 ].", GH_ParamAccess.item, 0.5);
      pManager[3].Optional = true;  // OM
    }

    protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager) {
      pManager.AddLineParameter("Organic Matter Inner", "OM-inner",
                                "Lines as inner soil organic matter.", GH_ParamAccess.tree);
      pManager.AddGenericParameter(
          "Organic Matter Property", "OM-prop",
          "Property of inner organic matter to generate top organic matter.", GH_ParamAccess.item);
    }
    ...
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilOrganicsInner);
  ...
  public class BALsoilOrganicMatterTop : GH_Component {
    public BALsoilOrganicMatterTop()
        : base("Soil Surface Organic Matter (independent version)", "balSoilOG_topInd",
               "Generate soil surface organic matter based on given intensity.", "BAL",
               "01::soil") {}
    ...
    protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager) {
      pManager.AddGenericParameter("Soil Base", "soilBase",
                                   "The base object used for soil diagram generation.",
                                   GH_ParamAccess.item);
      pManager.AddIntegerParameter("numLayer", "numL", "Layer number of surface organic matter",
                                   GH_ParamAccess.item, 1);
    ...
    protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager) {
      pManager.AddLineParameter("Organic Matter Top", "soilOM-Top",
                                "Curves representing organic matter on soil surface.",
                                GH_ParamAccess.tree);
    }
    ...
    public override void AppendAdditionalMenuItems(ToolStripDropDown menu) {
      ...
      Menu_AppendSeparator(menu);
      Menu_AppendItem(menu, "OM Size:", (sender, e) => {}, false).Font =
          GH_FontServer.StandardItalic;
      Menu_AppendItem(menu, " S", (sender, e) => Menu.SelectMode(this, sender, e, ref omSize, "S"),
                      true, CheckMode("S"));
      Menu_AppendItem(menu, " M", (sender, e) => Menu.SelectMode(this, sender, e, ref omSize, "M"),
                      true, CheckMode("M"));
      Menu_AppendItem(menu, " L", (sender, e) => Menu.SelectMode(this, sender, e, ref omSize, "L"),
                      true, CheckMode("L"));
    }
    ...
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilOrganicsTop);
  ...
  public class BALsoilOrganicMatterTopAlter : GH_Component {
    public BALsoilOrganicMatterTopAlter()
        : base(
              "Soil Surface Organic Matter (dependent version)", "balSoilOG_topDep",
    ...
    protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager) {
      pManager.AddGenericParameter(
          "Organic Matter Property", "OM-prop",
          "Organic matter property from soil inner organic matter component.", GH_ParamAccess.item);
      pManager.AddIntegerParameter("numLayer", "numL", "Layer number of surface organic matter",
                                   GH_ParamAccess.item, 1);
      pManager[1].Optional = true;
    }
    ...
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilOrganicsTopDep);
  ...
  public class BALsoilCompaction : GH_Component {
    public BALsoilCompaction()
        : base("Soil Compaction", "balSoilComp",
    ...
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balSoilCompact);
    public override GH_Exposure Exposure => GH_Exposure.tertiary;

    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      pManager.AddGenericParameter("Soil Base", "soilBase", "Soil base triangle map.",
                                   GH_ParamAccess.item);
      pManager.AddCurveParameter("Soil Triangle", "soilTri",
                                 "Soil triangles, representing the initial soil separates divsion.",
                                 GH_ParamAccess.list);
      pManager.AddCurveParameter(
          "Soil Core", "soilCore",
          "Soil core triangles, representing soil separates without any water.",
      ...
      pManager.AddCurveParameter(
          "Compact Area", "cpArea",
          "Compaction area near the soil surface. Using lines or curves for the input.",
          GH_ParamAccess.item);
      pManager.AddNumberParameter("Compact Depth", "cpDepth",
                                  "Compaction Depth, unit associated with Rhino's unit.",
                                  GH_ParamAccess.item);
      pManager.AddNumberParameter(
          "Strength", "strength",
          "Compaction strength based on your model unit. The range should be [0, 1] for [m] and [0, 1000] for [mm].",
          GH_ParamAccess.item, 0.5);
      pManager[pManager.ParamCount - 1].Optional = true;
    }

    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      pManager.AddCurveParameter("Soil Core Compacted", "soilCoreCmpc",
                                 "Soil core separates after compaction.", GH_ParamAccess.list);
      pManager.AddCurveParameter("Soil Current Water Compacted", "soilCWCmpc",
                                 "Water content after compaction.", GH_ParamAccess.list);

      pManager.AddCurveParameter(
          "Soil Core Non-Compacted", "soilCoreNonCmpc",
          "Soil core separates after compaction outside the compaction area.", GH_ParamAccess.list);
      pManager.AddCurveParameter("Soil Current Water Non-Compacted", "soilCWNonCmpc",
                                 "Water content after compaction outside the compaction area.",
                                 GH_ParamAccess.list);

      pManager.AddCurveParameter("Affected Boundary", "affectedBound",
                                 "Boundary of the affected soil after compaction.",
                                 GH_ParamAccess.item);
    }
...
#region processing input
      ...
      if (soilTriCrv.Count != coreTriCrv.Count) {
        AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                          "Soil core triangles and soil triangles need to have the same number.");
        return;
      }
      ...
      var compactBnd =
          new Polyline(new List<Point3d> { p0, p0Down, p1Down, p1, p0 }).ToNurbsCurve();
...
// var compactBnd = new Polyline(bndPtLst).ToNurbsCurve();
      ...
      if (triDict.Count == 0) {
        AddRuntimeMessage(GH_RuntimeMessageLevel.Warning,
                          "No soil are affected by the compaction.");
        return;
      }
      ...
      foreach (var x in triDict) {
        ...
        // the local strength is based on the distance to the compactCrv and the size of the
        // triangle
        ...
        curCoreT.Transform(
            Transform.Translation(dir * transStrength * globalStrength * adjustedScale));
        coreTriCmpc.Add(curCoreT);

        // water offset depends on the local compression strength -- more it is compressed, less
        // water it can hold
        if (hasWater) {
          ...
          curBndT.Transform(
              Transform.Scale((curBndT[0] + curBndT[1] + curBndT[2]) / 3, 1 / coreRatio));

          // var scaleRatio = 1 - Utils.remap(rWater, 0, globalStrength * scaleStrengh, coreRatio,
          // 1);
      ...
      // var coreTriNonCmpc = coreTri.Where(x =>
      // !triDict.Keys.Contains(coreTri.IndexOf(x))).ToList();
    ...
    //   Menu_AppendItem(menu, "Mode:", (sender, e) => { }, false).Font =
    //   GH_FontServer.StandardItalic;

    //  Menu_AppendItem(menu, "Global", (sender, e) => Menu.SelectMode(this, sender, e, ref appMode,
    //  "Global"), true, CheckMode("Global")); Menu_AppendItem(menu, "Local", (sender, e) =>
    //  Menu.SelectMode(this, sender, e, ref appMode, "Local"), true, CheckMode("Local"));
    //}
