...
namespace BeingAliveLanguage
{
  ...
  {
    public BALtree2d()
        : base("Tree_2D", "balTree_2D", "Generate the 2D version of BAL tree.", "BAL", "03::plant") { }

    // string modeUnitary = "non-unitary";
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balTree2D);
    ...
    protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
    {
      pManager.AddPlaneParameter("Plane",
                                 "P",
                                 "Base plane(s) where the tree(s) is drawn.",
                                 GH_ParamAccess.list,
                                 Plane.WorldXY);
      ...
      pManager.AddNumberParameter("TopBranchAngle",
                                  "tbAng",
                                  "Opening angle of top branches in degrees (5-60).",
                                  GH_ParamAccess.list,
                                  35.0);
    ...
    protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager)
    {
      ...
      pManager.AddCurveParameter("CanopyCurve",
                                 "CC",
                                 "A curve on top of the tree that represents its canopy.",
                                 GH_ParamAccess.tree);
      ...
      pManager.AddGenericParameter(
          "TreeInfo", "Tinfo", "Information about the tree.", GH_ParamAccess.list);
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
      ...
      var sideAngLst = new List<double>();
      DA.GetDataList("SideBranchAngle", sideAngLst);

      var topAngLst = new List<double>();
      DA.GetDataList("TopBranchAngle", topAngLst);
      ...
      else if (plnLst.Count > 1)
      {
        ...
        else if (hLst.Count != plnLst.Count)
        {
          AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                            "Height # does not match Plane #, please check.");
          return;
        }
        ...
        {
          AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                            "Phase # does not match Plane #, please check.");
          return;
        }

        // Handle angle list - replicate single value or validate count
        if (sideAngLst.Count == 0)
          sideAngLst = Enumerable.Repeat(95.0, plnLst.Count).ToList();
        else if (sideAngLst.Count == 1)
          sideAngLst = Enumerable.Repeat(sideAngLst[0], plnLst.Count).ToList();
        else if (sideAngLst.Count != plnLst.Count)
        {
          AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
        ...
        if (topAngLst.Count == 0)
          topAngLst = Enumerable.Repeat(35.0, plnLst.Count).ToList();
        else if (topAngLst.Count == 1)
          topAngLst = Enumerable.Repeat(topAngLst[0], plnLst.Count).ToList();
        else if (topAngLst.Count != plnLst.Count)
        {
          AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                            "TopBranchAngle # does not match Plane #, please check.");
          return;
        }

        // Validate angle range
        foreach (var ang in sideAngLst)
        {
          if (ang < 30 || ang > 120)
          {
        ...
        foreach (var ang in topAngLst)
        {
          if (ang < 5 || ang > 60)
          {
            AddRuntimeMessage(GH_RuntimeMessageLevel.Warning,
                              "TopBranchAngle should be within [5, 60]. Values will be clamped.");
            break;
          }
        }
        ...
        for (int i = 0; i < plnLst.Count; i++)
        {
          ...
          double clampedSideAngle = Math.Max(30, Math.Min(120, sideAngLst[i]));
          double clampedTopAngle = Math.Max(5, Math.Min(60, topAngLst[i]));
          var t = new Tree2D(plnLst[i], hLst[i], false, clampedSideAngle, clampedTopAngle);
        ...
        for (int i = 0; i < distLst.Count; i++)
        {
          ...
          inbetweenScale.Add(
              Math.Min(1, distLst[i] / ((widCol[firstIdx] + widCol[secondIdx]) * 0.5)));
        }
      ...
      else if (plnLst.Count == 1)  // special case: only one tree
      {
        ...
        double singleSideAngle = sideAngLst.Count > 0 ? sideAngLst[0] : 95.0;
        singleSideAngle = Math.Max(30, Math.Min(120, singleSideAngle));
        double singleTopAngle = topAngLst.Count > 0 ? topAngLst[0] : 35.0;
        singleTopAngle = Math.Max(5, Math.Min(60, singleTopAngle));

        tscal.Add(Tuple.Create(1.0, 1.0));
        treeCol.Add(new Tree2D(plnLst[0], hLst[0], false, singleSideAngle, singleTopAngle));
  ...
  public class BALtree3dComposer : GH_Component
  {
    ...
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balTreeComposer);
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager)
    {
      ...
      pManager.AddNumberParameter(
          "GlobalScale", "globalS", "Global scale of the tree.", GH_ParamAccess.item, 1);
      pManager.AddNumberParameter(
          "TrunkScale", "trunkS", "Trunk scale of the tree.", GH_ParamAccess.item, 1);
      pManager.AddNumberParameter("SpreadAngleMain",
                                  "angMain",
                                  "Spread angle of the primary tree branches.",
                                  GH_ParamAccess.item,
                                  50);
      pManager.AddNumberParameter("SpreadAngleTop",
                                  "angTop",
                                  "Spread angle of the secontary tree branches (the top part).",
                                  GH_ParamAccess.item,
                                  35);
      pManager.AddIntegerParameter(
          "Phase", "phase", "Phase of the tree's growth.", GH_ParamAccess.item);
      pManager.AddIntegerParameter(
          "Seed", "seed", "Seed for random number to varify the tree shape.", GH_ParamAccess.item, 0);
      pManager.AddBooleanParameter("BranchRotation",
                                   "brRot",
                                   "Whether to rotate the branches sequentially.",
                                   GH_ParamAccess.item,
                                   false);
      pManager.AddIntegerParameter("DuplicateNumber",
                                   "dupNum",
                                   "[0-3] Number of top side branches for duplicate branching.",
                                   GH_ParamAccess.item,
                                   0);
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager)
    {
      pManager.AddGenericParameter(
          "Tree Objects",
          "Tree_Out",
          "Tree objects that can be passed to renderer or interaction components.",
          GH_ParamAccess.item);
    }
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
      ...
      if (dupNum < 0 || dupNum > 3)
        AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                          "DuplicateNumber is out of range [0, 3], please check.");
  ...
  public class BALtree3dRenderer : GH_Component
  {
    ...
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balTreeRenderer);
    public override Guid ComponentGuid => new Guid("fb920ed6-7c50-466b-8645-04eb5658a7f1");

    protected override void RegisterInputParams(GH_InputParamManager pManager)
    {
      pManager.AddGenericParameter(
          "Tree Objects", "Tree_In", "Tree objects generated by to render.", GH_ParamAccess.tree);
      pManager.AddTextParameter(
          "Id",
          "id",
          "An optional id of the tree or tree group, used for selecting trees in the renderer.",
          GH_ParamAccess.item,
          "");
    }
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager)
    {
      ...
      pManager.AddCurveParameter(
          "SideBranch", "SB", "Tree side branch curves (non-split).", GH_ParamAccess.tree);
      pManager.AddCurveParameter("TopBranch",
                                 "TB",
                                 "Tree top branch and duplicated branch curves (splitted).",
                                 GH_ParamAccess.tree);
      pManager.AddGenericParameter(
          "TreeInfo", "Tinfo", "Information about the tree.", GH_ParamAccess.tree);
    }
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
      ...
      foreach (GH_Path inputPath in inputTree.Paths)
      {
        ...
        for (int treeIdx = 0; treeIdx < trees.Count; treeIdx++)
        {
          ...
          {
            AddRuntimeMessage(GH_RuntimeMessageLevel.Warning,
                              "Some tree object from the input is invalid.");
            continue;
          }
          ...
          treeInfoTree.Add(new TreeProperty(treeWrapper.Tree.mPln,
                                            treeWrapper.Phase,
                                            treeWrapper.Height,
                                            treeWrapper.Radius,
  ...
  public class BALtreeInteraction : GH_Component
  {
    ...
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balTreeInteraction);
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager)
    {
      pManager.AddGenericParameter(
          "T -> Interaction",
          "tToInteract",
          "The collection of tree objects in various species before interaction.",
          GH_ParamAccess.list);
    }
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager)
    {
      pManager.AddGenericParameter(
          "Interaction -> T",
          "tAfterInteract",
          "The collection of tree objects in various species after interaction.",
          GH_ParamAccess.list);
    }
  ...
  public class BALtreeEnergyCanopy : GH_Component
  {
    ...
    protected override System.Drawing.Bitmap Icon =>
        SysUtils.cvtByteBitmap(Properties.Resources.balTree3DVolume);
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager)
    {
      ...
      pManager.AddNumberParameter("TrunkRadius",
                                  "T-r",
                                  "Tree trunk radius, for creating different sized trunk.",
                                  GH_ParamAccess.tree,
                                  1);
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager)
    {
      pManager.AddMeshParameter(
          "EnergyVolume", "E", "Energy volume for energy analysis.", GH_ParamAccess.tree);
    }
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
      ...
      {
        AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                          "The path count of Branch does not match the path count of Trunk.");
        return;
      }
