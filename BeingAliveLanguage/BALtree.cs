...
namespace BeingAliveLanguage {
  ...
  public class BALtree2d : GH_Component {
    ...
    protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager) {
      pManager.AddPlaneParameter("Plane", "P", "Base plane(s) where the tree(s) is drawn.",
  ...
  public class BALtree3dComposer : GH_Component {
    public BALtree3dComposer()
        : base("Tree_3D_Composer", "balTree_3dComposer", "Compose the 3D version of BAL tree.",
               "BAL", "03::plant") {}
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      ...
      pManager.AddNumberParameter("GlobalScale", "globalS", "Global scale of the tree.",
                                  GH_ParamAccess.item, 1);
      pManager.AddNumberParameter("TrunkScale", "trunkS", "Trunk scale of the tree.",
                                  GH_ParamAccess.item, 1);
      pManager.AddNumberParameter("SpreadAngleMain", "angMain",
                                  "Spread angle of the primary tree branches.", GH_ParamAccess.item,
                                  50);
      pManager.AddNumberParameter("SpreadAngleTop", "angTop",
                                  "Spread angle of the secontary tree branches (the top part).",
                                  GH_ParamAccess.item, 35);
      pManager.AddIntegerParameter("Phase", "phase", "Phase of the tree's growth.",
                                   GH_ParamAccess.item);
      pManager.AddIntegerParameter("Seed", "seed",
                                   "Seed for random number to varify the tree shape.",
                                   GH_ParamAccess.item, 0);
      pManager.AddBooleanParameter("BranchRotation", "brRot",
                                   "Whether to rotate the branches sequentially.",
                                   GH_ParamAccess.item, false);
      pManager.AddIntegerParameter("DuplicateNumber", "dupNum",
                                   "[0-3] Number of top side branches for duplicate branching.",
                                   GH_ParamAccess.item, 0);
      pManager.AddTextParameter(
          "Id", "id",
          "An optional id of the tree or tree group, used for selecting trees in the same group in the renderer.",
          GH_ParamAccess.item, "");
    }

    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      pManager.AddGenericParameter(
          "Tree Objects", "Tree_Out",
...
#region IGH_Goo implementation
  ...
  public class BALtree3dRenderer : GH_Component {
    public BALtree3dRenderer()
        : base("Tree_3D_Renderer", "balTree_3dRender", "Render the 3D version of BAL tree.", "BAL",
               "03::plant") {}
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      pManager.AddGenericParameter("Tree Objects", "Tree_In",
                                   "Tree objects generated by to render.", GH_ParamAccess.tree);
      pManager.AddTextParameter(
          "Id", "id",
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      pManager.AddCurveParameter("Trunk", "T", "Tree trunk curves.", GH_ParamAccess.tree);
      pManager.AddCurveParameter("SingleBranch", "SB", "Tree side branch curves (non-split).",
                                 GH_ParamAccess.tree);
      pManager.AddCurveParameter("SplitBranch", "TB",
    ...
    protected override void SolveInstance(IGH_DataAccess DA) {
      ...
      foreach (GH_Path inputPath in inputTree.Paths) {
        ...
        for (int treeIdx = 0; treeIdx < trees.Count; treeIdx++) {
          ...
          treeInfoTree.Add(
              new TreeProperty(treeWrapper.Tree.mPln, treeWrapper.Phase, treeWrapper.Height,
                               treeWrapper.Radius, treeWrapper.Tree.mScaledLen),
              treePath);
        }
      }
    ...
    protected override void SolveInstance(IGH_DataAccess DA) {
      ...
      if (nearestNeighbors.Count == 0) {
        AddRuntimeMessage(
            ...
            "No forest interaction happens. Either trees have no neighbour, or something wrong happens.");
  ...
  public class BALtreeEnergyCanopy : GH_Component {
    public BALtreeEnergyCanopy()
        : base("Tree Energy Canopy", "balEnergyCanopy",
               "Generate the energy canopy of the 3D tree for energy analysis.", "BAL",
               "03::plant") {}
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      ...
      pManager.AddNumberParameter("TrunkRadius", "T-r",
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      pManager.AddMeshParameter("EnergyVolume", "E", "Energy volume for energy analysis.",
                                GH_ParamAccess.tree);
    }
...
#region input check
      ...
      for (int i = 0; i < branchCol.PathCount; i++) {
        ...
        if (!energyMesh.IsValid)
          AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                            "Canopy mesh is not valid. Please check.");
